...
namespace DocumentManagement.API.Controllers.ManageWorkFlow
{
    ...
    [ApiController]
    [Authorize]
    public class MyApprovedFileController : BaseController
    {
        public IMediator _mediator { get; set; }
        public MyApprovedFileController(IMediator mediator)
        {
            _mediator = mediator;
        }
        [HttpGet]
        [Produces("application/json", "application/xml", Type = typeof(WorkFlowList))]
        public async Task<IActionResult> GetMyApprovedFiles([FromQuery] WorkFlowResource documentResource)
        {
            var getDocumentLibraryQuery = new GetMyApprovedFileLibraryQuery
            {
